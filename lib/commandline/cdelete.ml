...
struct
  ...
  let term_peers = 
    Arg.(
      value
      & pos_all string []
      & info ~docv:"<PEERS>" ~doc:"Peers to delete" []
    )

    let term_cmd run = 
      let combine peers = run {peers}
    in
    ...
    let exec ~key peers angou = 
      let {peers} = peers in
      let deleted, angou = LibangouI.Peers.partition (fun s _ -> List.mem s peers) angou in
      match LibangouI.Peers.PeersMap.is_empty deleted with
        | true ->  Printf.eprintf "No peers deleted"
        | false -> 
